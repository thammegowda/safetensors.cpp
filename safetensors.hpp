...
namespace safetensors {
  ...
  // Constant dtype mappings - forward map (string -> ScalarType)

  static const std::unordered_map<std::string, torch::ScalarType> str_to_dtype = {
        {"BOOL", torch::kBool},
        {"U8", torch::kUInt8},
        {"I8", torch::kInt8},
        {"U16", torch::kUInt16},
  ...
  // Constant dtype mappings - reverse map (ScalarType -> string)
  // Generated from str_to_dtype to avoid duplication
  static const std::unordered_map<torch::ScalarType, std::string> dtype_to_str = []() {
    std::unordered_map<torch::ScalarType, std::string> result;
